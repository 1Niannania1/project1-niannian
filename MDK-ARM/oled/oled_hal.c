...
oledFont* oledfont = &SongFont_ASCII_5x8;
...
void oled_drawChar(int16_t x, int16_t y, char c, uint8_t color, uint16_t bg, uint8_t size){
	uint8_t _fontW = oledfont->width;
	uint8_t _fontH = oledfont->height;
	int8_t i;
	int8_t j;
	int8_t k;
...
void oled_drawText(int16_t x, int16_t y, char* text){
	...
	uint8_t c = text[0];
	uint8_t _w = oledfont->width;
	uint8_t _h = oledfont->height;
  if(oledfont) { // 'Classic' built-in font
  	while(c != '\0'){
      if(c == '\n') {                        // Newline?
